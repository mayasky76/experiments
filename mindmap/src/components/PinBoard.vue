<template>
  <div id="PinBoard" ref="PinBoard" @mousemove="moveCursor">
    <div v-for="(item, index) in LinkData" :key="index" class="lineDraw" :style="lineStyle(index)"></div>
    <div v-for="(item, index) in BoardData" :key="index">
      <div
        ...
        @mouseleave="clearLink($event,item)"
      ...
      >
        ...
        <div v-if="item.type === 'image'">
          ...
          >
            ...
            <polygon fill="#FFFFFF" points="9,19 12,16 13,17 16,14 21,19 " />
            <circle fill="#FFFFFF" cx="12" cy="13" r="1" />
...
export default {
  ...
  methods: {
    ...
    lineStyle(key) {
      var off1 = {
        top: parseInt(this.BoardData[this.LinkData[key].start].top, 10),
        left: parseInt(this.BoardData[this.LinkData[key].start].left, 10),
        width: 600,
        height: 25
      };
      var off2 = {
        top: parseInt(this.BoardData[this.LinkData[key].end].top, 10),
        left: parseInt(this.BoardData[this.LinkData[key].end].left, 10),
        width: 600,
        height: 25
      };
    ...
    GoImage: function(e, item) {
      ...
      fileReader.onload = function(event) {
        ...
        image.onload = function() {
          ...
          item.content = canvas.toDataURL();
          // this.setitem.content= canvas.toDataURL();
